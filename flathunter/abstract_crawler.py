...
class Crawler(ABC):
    ...
    def _save_debug_page(self, url: str, html_content: str, page_type: str = "search"):
        """Save HTML page to disk for debugging purposes"""
        if not self.config.debug_save_pages():
            return

        try:
            output_dir = self.config.debug_output_directory()
            crawler_name = self.get_name()
            crawler_dir = os.path.join(output_dir, crawler_name)
            ...
            if parsed_url.query:
                # Limit query string length to avoid overly long filenames
                query_hash = str(hash(parsed_url.query))[:8]
                filename = f"{timestamp}_{page_type}_{url_path}_{query_hash}.html"
            else:
                filename = f"{timestamp}_{page_type}_{url_path}.html"
            ...
            logger.info("Debug: Saved HTML page to %s", filepath)
        except Exception as e:  # pylint: disable=broad-except
            logger.warning("Failed to save debug page: %s", str(e))
    ...
    def get_soup_from_url(
            ...
            page_type: str = "search") -> BeautifulSoup:
        """Creates a Soup object from the HTML at the provided URL"""

        if self.config.use_proxy():
            return self.get_soup_with_proxy(url, page_type)
        if driver is not None:
            ...
            self._save_debug_page(url, driver.page_source, page_type)
        ...
        self._save_debug_page(url, html_content, page_type)
        return BeautifulSoup(resp.content, 'lxml')

    def get_soup_with_proxy(self, url, page_type: str = "search") -> BeautifulSoup:
